...
{
    $route = str_replace('admin.', '', $param);
    $permission = collect(Cache::get('admin_side_menu')->pluck('children')->flatten())   //flatten is turn associative to normal array and damge attribte
        ->where('as', $route)->flatten()->first();
    return $permission ? $permission['parent'] : null;    // permission become array not obj
}
